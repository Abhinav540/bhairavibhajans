...
function ContentCards({ upcomingProgram }: { upcomingProgram: Program | null }) {
  const upcomingDate = upcomingProgram ? new Date(`${upcomingProgram.date}T00:00:00`) : null;
    ...
    <section className="content-grid">
      ...
      <article className="panel program" id="programs">
        ...
        <div className="program-info">
          {upcomingProgram?.image && (
            <div className="program-image">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={upcomingProgram.image} alt={upcomingProgram.title} />
            </div>
          )}
          {upcomingDate && (
            <time>
              <small>{upcomingDate.toLocaleString("en-GB", { month: "short" }).toUpperCase()}</small>
              <strong>{String(upcomingDate.getDate()).padStart(2, "0")}</strong>
            </time>
          )}
          <div>
            <h3>{upcomingProgram?.title ?? "Check our schedule"}</h3>
            <p>
              {upcomingProgram ? (
                <>
                  <MapPin size={13} /> {upcomingProgram.location ?? "Kerala"}
                </>
              ) : (
      ...
      <article className="panel video" id="videos">
        ...
        >
          <Image src={latestVideoThumbnail} alt="Bhairavi Bhajans at Oru Chiri Iru Chiri Bumber Chiri" fill sizes="280px" />
...
export default function HomePage({ upcomingProgram }: { upcomingProgram: Program | null }) {
  return (
    <>
      <Navbar active="Home" />
      <main>
        <Hero />
        <ContentCards upcomingProgram={upcomingProgram} />
        <StageSection />
      </main>
      <Footer />
    </>
